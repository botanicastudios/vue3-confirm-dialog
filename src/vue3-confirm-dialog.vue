                ...
                <div v-if="isShow" ref="vueConfirmDialog" class="vc-container">
                    <span class="vc-text-grid">
                        ...
                        <span v-if="dialog.auth">
                            <input v-focus v-model="password" class="vc-input" type="password" name="vc-password"
                                placeholder="Password" autocomplete="off" />
                        </span>
                    </span>
                    <div class="vc-btn-grid" :class="{ isMono: !dialog.button.no || !dialog.button.yes }">
                        <button v-if="dialog.button.no" @click.stop="e => handleClickButton(e, false)"
                            class="vc-btn left" type="button">
                            {{ dialog.button.no }}
                        </button>
...
export default {
    setup() {
        ...
        onMounted(() => {
            ...
            //assign functions to EventBus events
        ...
        //on open, resets all values for the new ones to be loaded
        function resetState() {
            password.value = null
        ...
        //on click on the generated dialog buttons
        //on YES call callback with "true" as confirm
        function handleClickButton({ target }, confirm) {
        ...
        //on click on the background behind the modal (vue confirm)
        //call callback with "false" as confirm
        function handleClickOverlay({ target }) {
            if (target.id == 'vueConfirm') {
                isShow.value = false
                // callback
                if (params.callback) {
                    params.callback(false, password.value)
                }
            }
        }

        //close on Escape, approve on Enter if not disabled
        function handleKey(e) {
            if (!isShow.value || params.disableKeys) {
                return
            }
            ...
            if (e.key == 'Enter') {
                handleClickButton({ target: { id: '' } }, true)
            }
        }

        function open(paramsInput) {
            resetState()
